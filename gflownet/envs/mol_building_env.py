...
class MolBuildingEnvContext:
    ...
    def __init__(self, atoms=['H', 'C', 'N', 'O', 'F']):
        ...
        self.bond_attr_values = {
            'type': [BondType.SINGLE, BondType.DOUBLE, BondType.TRIPLE],
        }
    ...
    def aidx_to_GraphAction(self, g: gd.Data, action_idx: tuple[int, int, int], t: GraphActionType):
        ...
        if t is GraphActionType.Stop:
            ...
            return GraphAction(t, source=a, target=b)
            ...
            return GraphAction(t, source=a, target=b, attr=attr, value=val)

    def graph_to_Data(self, g):
        x = torch.zeros((max(1, len(g.nodes)), self.atom_attr_size))
        for i in g.nodes:
            for k, sl in zip(self.atom_attrs, self.atom_attr_slice):
                idx = self.atom_attr_values[k].index(g.nodes[i][k]) if k in g.nodes[i] else 0
                x[i, sl + idx] = 1
        edge_attr = torch.zeros((len(g.edges) * 2, self.bond_attr_size))
        ...
        edge_index = torch.tensor([e for i, j in g.edges for e in [(i, j), (j, i)]]).reshape((-1, 2)).T
        gc = nx.complement(g)
        non_edge_index = torch.tensor([i for i in gc.edges]).T
    ...
    def mol_to_graph(self, mol):
        ...
        for b in mol.GetBonds():
            ...
            g.add_edge(b.GetBeginAtomIdx(), b.GetEndAtomIdx(),
                       **{attr: val for attr, val in attrs.items() if val != self.edge_attr_defaults[attr]})
        return g
