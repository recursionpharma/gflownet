...
def get_jnk3_gsk3b_mlp():
    ...
    return jnk3_gsk3b_mlp


def load_jnk3_gsk3b_model():
    jnk3_gsk3b_model = get_jnk3_gsk3b_mlp()
    jnk3_gsk3b_model.load_state_dict(torch.load(gzip.open(Path(__file__).parent / "jnk3_gsk3b_mlp.pt.gz", "rb")))
    return jnk3_gsk3b_model
...
def get_data():
    ...
    # Get ECFP6 (radius=3)
    fps = [Chem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(i), radius=3, nBits=2048) for i in smis]
...
if __name__ == "__main__":
    ...
    Xs, Ys = get_data()
    jnk3_gsk3b_mlp = get_model().to(dev)
