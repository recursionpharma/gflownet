...
from gflownet.envs.graph_building_env import Graph, GraphActionType, GraphBuildingEnv
...
class CliqueDataset(Dataset):
    def __init__(
        ...
        if modal_seed is None:
            ...
            rng = np.random.default_rng(split_seed + modal_seed)
            is_mode_present = np.bool_(
                np.zeros(mode_labels.max() + 1)
            )  # rng.uniform(size=mode_labels.max() + 1) < ratio
...
class CliqueTask(GFNTask):
    ...
    def cond_info_to_logreward(self, cond_info: Dict[str, Tensor], flat_reward: FlatRewards) -> RewardScalar:
        ...
        return RewardScalar(scalar_reward * cond_info["beta"]).float()
...
class CliquesTrainer(GFNTrainer):
    ...
    def setup(self):
        ...
        self.training_data = CliqueDataset(
            self._data,
            self.ctx,
            4,
            train=True,
            ratio=hps.get("train_ratio", 0.9),
            modal_seed=hps.get("modal_seed", None),
            max_nodes=max_nodes,
        )
        self.test_data = CliqueDataset(
            self._data,
            self.ctx,
            4,
            train=False,
        ...
        self.task = CliqueTask(
            self.training_data,
            hps["temperature_sample_dist"],
            ast.literal_eval(str(hps["temperature_dist_params"])),
        ...
        if split_type == "random":
            ...
            train_idcs, test_idcs = self.exact_prob_cb.get_subtree_test_split(
                hps.get("train_ratio", 0.9), hps.get("test_split_seed", 142857)
            )
            self.training_data.idcs = train_idcs
            self.test_data.idcs = test_idcs
