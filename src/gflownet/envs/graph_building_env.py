...
class GraphActionCategorical:
    ...
    def logsoftmax(self):
        ...
        maxl = (
            torch.cat(
                [
                    scatter(i.detach(), b, dim=0, out=out, reduce="max")
                    for i, b, out in zip(self.logits, self.batch, outs)
                ],
                dim=1,
            )
            .max(1)
    ...
    def logsumexp(self, x=None):
        ...
        maxl = (
            torch.cat([scatter(i, b, dim=0, out=out, reduce="max") for i, b, out in zip(x, self.batch, outs)], dim=1)
            .max(1)
            .values.detach()
        )
        # substract by max then take exp
