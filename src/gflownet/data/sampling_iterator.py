...
from gflownet.utils.multiprocessing_proxy import put_into_batch_buffer, SharedPinnedBuffer
...
class SamplingIterator(IterableDataset):
    ...
    def __init__(
        ...
        num_workers: int = 1,
        do_multiple_buffers = True, # If True, each worker has its own buffer; doesn't seem to have much impact either way
    ):
        """Parameters
        ----------
        dataset: Dataset
        ...
        self.result_buffer_size = buffer_size
        self.do_multiple_buffers = do_multiple_buffers
        if buffer_size and do_multiple_buffers:
            self.result_buffer = [SharedPinnedBuffer(buffer_size) for _ in range(num_workers)]
        elif buffer_size:
            self.result_buffer = SharedPinnedBuffer(buffer_size)
            self.round_robin_cond = mp.Condition()
            self.round_robin_counter = torch.zeros(1)
            self.round_robin_counter.share_memory_()
    ...
    def __iter__(self):
        ...
        for idcs in self._idx_iterator():
            ...
            if self.result_buffer_size and not self.do_multiple_buffers:
                with self.round_robin_cond:
                    self.round_robin_cond.wait_for(lambda: self.round_robin_counter[0] == self._wid)
                self.result_buffer.lock.acquire()
                yield put_into_batch_buffer(batch, self.result_buffer.buffer)
                self.round_robin_counter[0] = (self._wid + 1) % worker_info.num_workers
                with self.round_robin_cond:
                    self.round_robin_cond.notify_all()
            elif self.result_buffer_size:
