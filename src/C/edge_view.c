...
int _EdgeView_eq(EdgeView *self, EdgeView *other) {
    ...
    int num_settable = ((GraphDef*)self->graph->graph_def)->num_settable_edge_attrs;
    ...
    for (int i = 0; i < self->graph->num_edge_attrs; i++) {
        if (self->graph->edge_attrs[3 * i] == self->index) {
            self_values[self->graph->edge_attrs[3 * i + 1]] = self->graph->edge_attrs[3 * i + 2];
        }
    }
    for (int i = 0; i < other->graph->num_edge_attrs; i++) {
        if (other->graph->edge_attrs[3 * i] == other->index) {
            other_values[other->graph->edge_attrs[3 * i + 1]] = other->graph->edge_attrs[3 * i + 2];
        }
    }
    for (int i = 0; i < num_settable; i++) {
...
static PyObject*
EdgeView_richcompare(PyObject *_self, PyObject *other, int op) {
    PyObject *result = NULL;
    EdgeView *self = (EdgeView *)_self;

    if (other == NULL || other->ob_type != &EdgeViewType) {
        result = Py_NotImplemented;
    }
    else {
        if (op == Py_EQ){
            if (self->index == -1 || ((EdgeView *)other)->index == -1){
                PyErr_SetString(PyExc_TypeError, "EdgeView.__eq__ only supports equality comparison for bound EdgeViews");
                return NULL;
            }
            EdgeView *other_node = (EdgeView *)other;
            if (_EdgeView_eq(self, other_node) == 1){
                result = Py_True;
            } else {
                result = Py_False;
            }
        }else{
            PyErr_SetString(PyExc_TypeError, "EdgeView only supports equality comparison");
            return NULL;
        }
    }
...
PyTypeObject EdgeViewType = {
    ...
    .tp_richcompare = EdgeView_richcompare,
};
