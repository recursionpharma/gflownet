...
PyObject *mol_graph_to_Data(PyObject *self, PyObject *args) {
    ...
    /*for (int i=0;i<5;i++){
        printf("%d %d\n", gd->node_attr_offsets[i], gd->node_attr_offsets[i]-i);
    }*/
    for (int i = 0; i < g->num_nodes; i++) {
        ...
        for (int j = 0; j < 5; j++) {
            ...
            if (logit_slice_end == logit_slice_start)
                continue; // we cannot actually set this attribute
            if (j == v_idx)
                continue; // we cannot remove nor set 'v'
            // printf("Node attr %d %d = %d\n", j, i, _node_attrs[j][i]);
