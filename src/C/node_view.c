...
int _NodeView_eq(NodeView *self, NodeView *other) {
    ...
    int num_settable = ((GraphDef*)self->graph->graph_def)->num_settable_node_attrs + 1;
    int self_values[num_settable];
    int other_values[num_settable];
    for (int i = 0; i < num_settable; i++) {
        self_values[i] = -1;
        other_values[i] = -1;
    }
    for (int i = 0; i < self->graph->num_node_attrs; i++) {
        if (self->graph->node_attrs[3 * i] == self->graph->nodes[self->index]) {
            self_values[self->graph->node_attrs[3 * i + 1]] = self->graph->node_attrs[3 * i + 2];
        }
    }
    for (int i = 0; i < other->graph->num_node_attrs; i++) {
        if (other->graph->node_attrs[3 * i] == other->graph->nodes[other->index]) {
            other_values[other->graph->node_attrs[3 * i + 1]] = other->graph->node_attrs[3 * i + 2];
        }
    }
    for (int i = 0; i < num_settable; i++) {
        // printf("%d %d %d\n", i, self_values[i], other_values[i]);
...
static PyObject *
NodeView_richcompare(PyObject *_self, PyObject *other, int op)
{
    PyObject *result = NULL;
    NodeView *self = (NodeView *)_self;

    if (other == NULL || other->ob_type != &NodeViewType) {
        result = Py_NotImplemented;
    }
    else {
        if (op == Py_EQ){
            if (self->index == -1 || ((NodeView *)other)->index == -1){
                PyErr_SetString(PyExc_TypeError, "NodeView.__eq__ only supports equality comparison for bound NodeViews");
                return NULL;
            }
            NodeView *other_node = (NodeView *)other;
            if (_NodeView_eq(self, other_node) == 1){
                result = Py_True;
            } else {
                result = Py_False;
            }
        }else{
            PyErr_SetString(PyExc_TypeError, "NodeView only supports equality comparison");
            return NULL;
        }
    }
...
PyTypeObject NodeViewType = {
    ...
    .tp_itemsize = 0,
    //.tp_flags = Py_TPFLAGS_HAVE_RICHCOMPARE, //Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE,
    .tp_new = NodeView_new,
    ...
    .tp_richcompare = NodeView_richcompare,
};
